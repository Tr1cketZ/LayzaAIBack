...
def perfil_update_completo(request):
    try:
        ...
        if user_serializer.is_valid():
            ...
            if any(key in request.data for key in ['pref_visual', 'pref_auditivo', 'pref_leitura_escrita']):
                perfil_serializer = PerfilUsuarioUpdateSerializer(
                    perfil,
                    data=request.data,
                    partial=request.method == 'PATCH'
                )
...
@swagger_auto_schema(
    ...
    manual_parameters=[
        ...
        openapi.Parameter(
            'data', openapi.IN_FORM, type=openapi.TYPE_STRING, format=openapi.FORMAT_DATE, required=True),
        openapi.Parameter(
